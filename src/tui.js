...
var CELL = '■';
function getMetricValue(heatmap) {
    return heatmap.totalValues;
}
function getMetricMax(heatmap) {
    return heatmap.maxValues;
}
function getCellValue(heatmap, row, column) {
...
function renderHeatmapPanel(heatmap, options) {
    ...
    var title = (_a = options.title) !== null && _a !== void 0 ? _a : 'Values';
    var total = getMetricValue(heatmap);
    var maxValue = getMetricMax(heatmap);
    var lines = [];
    lines.push("  ".concat(title, " (").concat(total, " total)"));
    lines.push("     ".concat(heatmap.columns.filter(function (_, index) { return index % 2 === 0; }).join('  ')));
    for (var rowIndex = 0; rowIndex < ROW_LABELS.length; rowIndex += 1) {
        ...
        for (var columnIndex = 0; columnIndex < heatmap.columns.length; columnIndex += 1) {
            var value = getCellValue(heatmap, rowIndex, columnIndex);
...
function parseArgs(argv) {
    ...
    return { path: path, analyzeOptions: analyzeOptions };
}
    ...
    return __awaiter(this, arguments, void 0, function (path, analyzeOptions, options) {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    records = _a.sent();
                    heatmap = (0, heatmap_js_1.buildHeatmap)(records.map(function (record) { return ({
...
function main() {
    return __awaiter(this, arguments, void 0, function (argv) {
        var _a, path, analyzeOptions;
        if (argv === void 0) { argv = process.argv.slice(2); }
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _a = parseArgs(argv), path = _a.path, analyzeOptions = _a.analyzeOptions;
                    return [4 /*yield*/, runTui(path, analyzeOptions)];
                case 1:
                    _b.sent();
                    return [2 /*return*/];
            }
